...
class Panel {
    constructor(maxLevel = 10, scoreToLevelUp = 10) {
        this.score = 0;
        this.level = 1;
...
class Snake {
    ...
    set X(value) {
        ...
        if (this.bodiesEle[1] && this.bodiesEle[1].offsetLeft === value) {
    ...
    set Y(value) {
        ...
        if (this.bodiesEle[1] && this.bodiesEle[1].offsetTop === value) {
    ...
    moveBody() {
        for (let i = this.bodiesEle.length - 1; i > 0; i -= 1) {
            let x = this.bodiesEle[i - 1].offsetLeft;
            let y = this.bodiesEle[i - 1].offsetTop;
            this.bodiesEle[i].style.left = x + 'px';
            this.bodiesEle[i].style.top = y + 'px';
        }
    }
    checkBody() {
        for (let i = 1; i < this.bodiesEle.length; i += 1) {
            const bodyEle = this.bodiesEle[i];
